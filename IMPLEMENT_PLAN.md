...
B7.14 v2 transparently records one pre-check directory filesystem-metadata incident (directory/filesystem metadata count 1; content/hash/date/return/validation counts 0) and locks one remaining real-container E1 structural preflight for `data/normalized/l1_yahoo_daily_v1.json`. The owner authorization permits only byte hashing and token-level schema/cutoff/symbol/session-date inspection after checkpoint exact-SHA CI. It must stop before return decoding, strategy computation, research decision, ledger write, or validation access. A passing preflight emits a date-only schedule attestation only; it does not authorize return execution.
