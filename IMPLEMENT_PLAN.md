...
B8.3 v4 is Inspector-accepted and supersedes B8.2 while preserving v1/v2/v3. B8.4R2/v3 is also Inspector-accepted after Exact-SHA CI run `30367686488`; it preserves CI-defective v1/v2 history, source-binds v4 without superseding its science, and provides E0 synthetic-only future falsification-preflight machinery. B8.5/v1 is immutable Inspector-rejected history because it left future machinery and storage resolution unlocked. B8.5R/v2 hash-binds the byte-level scanner, one-shot runner, report schema/validator, and synthetic structural envelope; future Phase B resolves only `LILY_DATA_ROOT` plus literal references and records observed raw manifest/payload hashes. Phase B is not executed and awaits Inspector acceptance. Every individual session must be structurally checked <= 2015-12-31 before return/value decoding and fail closed on missing/ambiguous U8 or date/schema/path/hash mismatch. `edge_claim none`; validation remains sealed.
...
| L-4 | Active planning E0; B8.3 and B8.4R2/v3 accepted; B8.5/v1 rejected immutable history, B8.5R/v2 hash-binds runnable structural machinery; `edge_claim none` | Phase B is not executed; wait for Inspector acceptance, then at most one structural U8 preflight only |
