...
B7.14 v2 transparently records one pre-check directory filesystem-metadata incident (directory/filesystem metadata count 1; content/hash/date/return/validation counts 0) and locks one remaining real-container E1 structural preflight for `data/normalized/l1_yahoo_daily_v1.json`. Incident provenance: the executor-recorded exact command is `Get-ChildItem -Force data\normalized | Select-Object Name,Length`; the Inspector's shorter directory-listing wording is a paraphrase, not an additional action. The owner authorization permits only byte hashing and token-level schema/cutoff/symbol/session-date inspection after checkpoint exact-SHA CI. It must stop before return decoding, strategy computation, research decision, ledger write, or validation access. A passing preflight emits a date-only schedule attestation only; it does not authorize return execution.
