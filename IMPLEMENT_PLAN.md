...
B8.6R9 Phase-A v11 supersedes Inspector-rejected B8.6R8/v10. It is E0 remediation only: the only future production entry is a git-show-sourced, stdlib-only bootstrap. Before project imports, marker claim, or dataset access, it verifies every declared runtime byte—including bootstrap, runner, contracts, opaque scanner, validators, schemas, and Draft subset—against the exact `producing_git_commit` blob and locked gate binding. No data access, activation, execution, or validation access occurred; `edge_claim none`, validation sealed, and Inspector review remains required.
...
| L-4 | Unresolved E0; consumed B8.5R5 remains no-retry, B8.6R6/v8 through B8.6R8/v10 are rejected history, and B8.6R9/v11 is a synthetic committed-bootstrap remediation pending Inspector review; `edge_claim none` | Inspector review, then a separately approved v11 activation checkpoint before one bounded provisioning attempt; validation remains sealed |
