...
B8.6R8 Phase-A v10 supersedes Inspector-rejected v9. It is E0 remediation only: before marker or dataset access, every declared execution dependency—including gate, runner, shared contract, opaque scanner, validators, schemas, and Draft subset—must equal its exact `producing_git_commit` blob and the gate source-binding identity. The report validator independently repeats this check, so a dirty or substituted implementation cannot claim reproducibility after the fact. No data access, activation, execution, or validation access occurred; `edge_claim none`, validation sealed, and Inspector review remains required.
...
| L-4 | Unresolved E0; consumed B8.5R5 remains no-retry, B8.6R6/v8 and B8.6R7/v9 are rejected history, and B8.6R8/v10 is a synthetic pre-read provenance remediation pending Inspector review; `edge_claim none` | Inspector review, then a separately approved v10 activation checkpoint before one bounded provisioning attempt; validation remains sealed |
