...
- **L-3 B7.14R5/v7 and B7.14R6/v8 rejected; B7.14R7/v9 in progress**: v7 remains immutable after failed Exact-SHA CI; v8 remains immutable after its duplicate-v7 manifest recovery sequence. v9 supersedes v8 once and proves every rejected predecessor only from exact committed blobs and portable addenda. Its LF-pinned synthetic scanner/runner/schemas/fixtures keep all fourteen authorizations false; validation is sealed, edge_claim is none, and L-3 remains E1 scope_restricted.
...
Keep validation returns sealed and do not rerun B4.11, B4.12, B7.3, B7.6, or B7.14. L-3 is E1 scope_restricted; B7.14R7/v9 is E0 synthetic-only and does not authorize a date-only inspection. The next safe action is v9 Phase-A exact-SHA CI; return execution remains not authorized. No UAT work is planned: a hostname reference is not a public UAT entitlement. Any future broker probe requires genuinely new official evidence or a dedicated owner-controlled test account, explicit owner approval, and a new locked gate.
