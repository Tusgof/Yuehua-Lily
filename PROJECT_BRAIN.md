...
- **L-3 B7.14 v3 rejected**: it hard-stopped at `unknown_structural_key` before session-date/return decoding, but its skipped timestamp path performed one UTF-8 text decode; Inspector rejects its permitted-scope representation. B7.14R/v4 is E0 synthetic machinery only; no further container access is authorized.
