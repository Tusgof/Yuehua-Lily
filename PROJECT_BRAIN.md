...
- **L-4 B8/B8.1/B8.2 preregistration**: B8.2 v3 supersedes prior gates without changing their bytes. The E0/no-data contract keeps U1 descriptive-only, fixes `u=q` (no volatility division), matched U4/U8, component-risk HHI, `N_eff`, per-metric three-plan MinTRL, deterministic controls, and exhaustive decision precedence. It creates no breadth evidence or `edge_claim none`; validation remains sealed and activation is not authorized.
