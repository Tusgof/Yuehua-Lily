...
- **L-3 B7.14 v3 and B7.14R/v4 rejected**: v3 hard-stopped at `unknown_structural_key` before session-date/return decoding, but its skipped timestamp path performed one UTF-8 text decode. v4 lacked exact artifact binding, complete nested schemas, both E0 modes, and first-row ledger-byte proof. B7.14R3/v5 is E0 synthetic-only machinery with those controls; no further container access is authorized.
