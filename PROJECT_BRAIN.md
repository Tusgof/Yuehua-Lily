...
- **L-4 B8.6R8 Phase A v10**: v9 is Inspector-rejected because it did not bind pre-read implementation bytes to the recorded producing commit. v10 is E0 remediation pending Inspector review: before marker claim or dataset open, the runner and report validator require every declared execution dependency (including scanner) to equal its `producing_git_commit` blob and its gate source binding. No dataset/container read, activation, execution, or validation access occurred; `edge_claim none` and validation sealed.
