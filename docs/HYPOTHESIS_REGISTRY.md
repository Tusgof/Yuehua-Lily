...
- **Status**: scope-restricted E0; B4.11 used its one authorized run but stopped after token create and two checks exhausted the three-request authentication cap, before any preview; minimum order, funding FX, execution quality, and realized costs remain unverified
...
- **Status**: scope-restricted E1; B4.9 closes the unavailable broker-ledger path, while the B4.11 authentication-cap stop adds no L-1 evidence; validation remains sealed and edge/historical-correctness claims remain unauthorized
