...
- **Status**: scope-restricted E1; B4.9 accepts that Webull Thailand has no published account-level corporate-action ledger, closes the B4.7 three-stream dry run without observation, and retains Alpha Vantage plus Lily Yahoo accounting as limited non-ground-truth streams; provider/broker calls, paper orders, validation access, edge, and historical-correctness claims remain unauthorized
