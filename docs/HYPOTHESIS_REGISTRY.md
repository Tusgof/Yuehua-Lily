...
- **B7.14 v2 checkpoint**: one pre-check directory filesystem-metadata incident is recorded transparently (metadata count 1; content/hash/date/return/validation counts 0). The executor-recorded exact command is `Get-ChildItem -Force data\normalized | Select-Object Name,Length`; the Inspector's shorter directory-listing wording is a paraphrase, not an additional action. One bounded E1 date-only preflight remains locked pending checkpoint CI. It may hash and structurally inspect only the exact designated container; no return/price decoding, strategy run, L-3 decision, ledger, or validation access is authorized.
