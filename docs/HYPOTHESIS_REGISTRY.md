...
- **B7.14 v2 checkpoint**: one pre-check directory filesystem-metadata incident is recorded transparently (metadata count 1; content/hash/date/return/validation counts 0), and one bounded E1 date-only preflight remains locked pending checkpoint CI. It may hash and structurally inspect only the exact designated container; no return/price decoding, strategy run, L-3 decision, ledger, or validation access is authorized.
