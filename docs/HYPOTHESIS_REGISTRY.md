...
- **B7.14 v3 rejected / B7.14R v4 rejected / B7.14R3 v5**: v3 hard-stopped at `unknown_structural_key` before session-date or return decoding, but one skipped timestamp lexeme was UTF-8 decoded; Inspector rejects its scope representation. Inspector also rejects v4 for unbound artifacts, open nested schemas, no `scope_restricted` matrix, incomplete adversarial cases, and an unproven ledger hash. v5 is E0 synthetic-only machinery: exact artifact/fixture hashes, closed nested contracts, both modes, first-row ledger-byte proof, and no further container access.
