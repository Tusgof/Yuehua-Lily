...
- **B7.14R5/v7 and B7.14R6/v8 rejected; B7.14R7/v9 in progress**: v7 stays immutable after failed Exact-SHA CI, and v8 stays immutable after the duplicate-v7 manifest recovery sequence. v9 supersedes v8 once at E0 only, binds v5–v8 only through exact committed blobs and portable addenda, and opens no data, container, execution, decision, ledger, or validation path. L-3 remains E1 `scope_restricted`, validation sealed, edge_claim none.
