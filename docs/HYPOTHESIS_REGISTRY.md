...
- **B7.14R5 v7 checkpoint**: v7 is in progress as E0-only synthetic/no-data remediation. Its exact cross-platform addendum records the v5/v6 authored-Windows and committed-LF identities plus failed Exact-SHA CI; it opens no data, container, execution, decision, ledger, or validation path. L-3 remains E1 `scope_restricted`, validation sealed, edge_claim none.
