...
- **B8.5R3 Phase A remediation v4**: v1/v2/v3 remain immutable Inspector-rejected history. v4 uses a capacity-derived bound, separates bounded prefix from complete raw provenance, preserves a first report on second invocation, requires a later machine-readable Inspector/CI activation record, and requires an explicit CLI flag. It remains E0; validation sealed, `edge_claim none`, and Phase B is not executed.
...
B8.4R2: L-4 remains edge_claim none; v3 synthetic E0 preflight active, validation sealed. B8.5/v1, B8.5R/v2, B8.5R2/v3, and B8.5R3/v4 are immutable rejected history; B8.5R4/v5 is the lifecycle-corrected structural E0 contract. Phase B and activation are not executed and await Inspector acceptance plus a later activation checkpoint.
