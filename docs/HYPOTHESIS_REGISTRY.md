...
- **B7.14 checkpoint**: one bounded E1 date-only preflight is locked pending checkpoint CI. It may hash and structurally inspect only the exact designated container; no return/price decoding, strategy run, L-3 decision, ledger, or validation access is authorized.
