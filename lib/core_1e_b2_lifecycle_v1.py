"""Fail-closed temporary-Git lifecycle for CORE-1E-B2 synthetic proof.

The project bootstrap is deny-only in this milestone.  The lifecycle below is
exercised only by a hermetic temporary Git repository with a test-installed
activation.  Activation, gate, CI-head, Git-blob, runtime-byte, cutoff,
container-identity, clean-checkout, and one-shot checks all happen before the
synthetic input is resolved or any return value is decoded.
"""
...
GATE_PATH = "experiments/core_1e_b2_development_execution_contract_v1.json"
ACTIVATION_PATH = "experiments/activation_records/core_1e_b2_activation_v1.json"
EXPECTED_GATE_ID = "core_1e_b2_development_execution_contract_v1"
EXPECTED_ACTIVATION_SCHEMA = "lily_core_1e_b2_activation_v1"
OWNER_AUTHORIZATION_REF = "owner_authorized_core_1e_b2_e0_machinery_only_2026-09-02"
...
ACTIVATION_SCHEMA_PATH = "schemas/core_1e_b2_activation_v1.schema.json"
CONTAINER_SCHEMA_PATH = NORMALIZED_SCHEMA_PATH
CONTRACT_VALIDATOR_PATH = "scripts/validate_core_1e_b2_development_execution_contract_v1.py"
REPORT_VALIDATOR_PATH = "scripts/validate_core_1e_b2_empirical_report_v1.py"
BOOTSTRAP_PATH = "scripts/run_core_1e_b2_committed_bootstrap_v1.py"
RUNTIME_PATHS = (
    ENGINE_PATH,
    ADAPTER_PATH,
    "lib/core_1e_b2_lifecycle_v1.py",
...
EXPECTED_ONE_SHOT_PATHS = {
    "marker_path": "reports/experiments/core_1e_b2_one_shot_marker_v1.json",
    "attempt_path": "reports/experiments/core_1e_b2_execution_attempt_v1.json",
    "report_path": "reports/experiments/core_1e_b2_execution_report_v1.json",
}
ACTIVATION_KEYS = {
    "schema_version",
    "gate_id",
...
def build_synthetic_activation(
    *,
    ...
    """Construct an activation for a temporary synthetic proof only."""
...
def _gate_blob_blockers(gate_raw: bytes | None) -> list[str]:
    ...
    blockers: list[str] = []
    if gate.get("schema_version") != "lily_core_1e_b2_development_execution_contract_v1":
        blockers.append("gate_schema_version_changed")
    if gate.get("order_id") != "CORE-1E-B2-A" or gate.get("work_order_id") != "CORE-1E-B2-A":
    ...
    if gate.get("normalized_container_contract") != expected_normalized:
        blockers.append("gate_normalized_container_contract_changed")
    boundaries = gate.get("execution_boundaries")
    expected_boundaries = {
        "mode": "synthetic_only",
        "allowed_input_ref": FIXTURE_PATH,
        "allowed_input_kind": "committed_fixture_only",
        "development_cutoff": DEVELOPMENT_CUTOFF,
        "reject_on_or_after": VALIDATION_CUTOFF,
    ...
    if boundaries != expected_boundaries:
        blockers.append("gate_execution_boundaries_changed")
    if gate.get("required_future_checks_before_input_decode") != EXPECTED_PRE_DECODE_CHECKS:
        blockers.append("gate_pre_decode_check_order_changed")
    if gate.get("report_contract", {}).get("schema_version") != REPORT_SCHEMA_VERSION:
        blockers.append("gate_report_schema_version_changed")
    if gate.get("report_contract", {}).get("schema_path") != SCHEMA_PATH:
        blockers.append("gate_report_schema_path_changed")
    return sorted(set(blockers))


def validate_activation(root: Path, head: str, activation: dict[str, Any]) -> list[str]:
    """Validate activation and committed provenance without touching input."""
...
def preflight(root: Path) -> dict[str, Any]:
    """Verify committed activation/lifecycle state without resolving input."""
...
def run_synthetic_once(
    ...
    *,
    ...
    marker = {
        "schema_version": "lily_core_1e_b2_marker_v1",
        "status": "claimed",
    ...
    try:
        ...
        # First parse with deferred numeric tokens.  Every date and structural
        # key is checked before decode_normalized_container converts a return.
        normalized, metadata = preflight_normalized_bytes(raw_input)
        decode_counter: MutableMapping[str, int] = {"count": 0}
        decode_normalized_bytes(raw_input, decode_counter=decode_counter)
        if decode_counter["count"] <= 0:
        ...
        attempt = {
            "schema_version": "lily_core_1e_b2_attempt_v1",
            "status": "completed",
            "input_read_count": 1,
