"""B7.12 closed-world synthetic derivation with deterministic locked boundaries."""
...
RELATIVE_MAXIMUM = Decimal("0.20")
EVENT_MAXIMUM_PERCENTAGE_POINTS = Decimal("10")
...
def decimal_value(value: Any) -> Decimal:
    if not finite(value, nonnegative=True):
...
def derive(observations: Any) -> tuple[dict[str, Any] | None, list[str]]:
    ...
    totals = {branch: {key: Decimal("0") for key in SIDE_KEYS} | {key: 0 for key in EVENT_KEYS} for branch in ("candidate", "comparator")}
    ...
    for expected, observation in enumerate(observations, start=1):
        ...
        try:
            for branch in ("candidate", "comparator"):
                row = side.get(branch)
                if not isinstance(row, dict) or set(row) != set(SIDE_KEYS) | set(EVENT_KEYS) or not all(type(row.get(key)) is bool for key in EVENT_KEYS): raise ValueError("side_effect_inputs")
                for key in SIDE_KEYS: totals[branch][key] += decimal_value(row.get(key))
                for key in EVENT_KEYS: totals[branch][key] += int(row[key])
        except ValueError:
    ...
    turnover_increase = (totals["candidate"]["turnover"] - totals["comparator"]["turnover"]) / totals["comparator"]["turnover"]
    cost_increase = (sum(totals["candidate"][key] for key in SIDE_KEYS[1:]) - comparator_cost) / comparator_cost
    event_increases = {key: Decimal("100") * (totals["candidate"][key] - totals["comparator"][key]) / Decimal(len(observations)) for key in EVENT_KEYS}
    limits_met = turnover_increase <= RELATIVE_MAXIMUM and cost_increase <= RELATIVE_MAXIMUM and all(value <= EVENT_MAXIMUM_PERCENTAGE_POINTS for value in event_increases.values())
    side = {"candidate": {key: float(value) if isinstance(value, Decimal) else value for key, value in totals["candidate"].items()}, "comparator": {key: float(value) if isinstance(value, Decimal) else value for key, value in totals["comparator"].items()}, "turnover_increase": float(turnover_increase), "cost_increase": float(cost_increase)}
    side.update({f"{key}_increase_percentage_points": float(value) for key, value in event_increases.items()})
