"""Closed-world helpers for B8.6R12/v14; no data is read here."""
...
MARKER = "reports/experiments/l_4_breadth_b86r12_provisioning_attempt_v14.json"
REPORT = "reports/experiments/l_4_breadth_b86r12_provisioning_report_v14.json"
MANIFEST = "experiments/provisioned/l_4_breadth_b86r12_falsification_manifest_v14.json"
PAYLOAD = "experiments/provisioned/l_4_breadth_b86r12_u8_session_dates_v14.json"
REPORT_SCHEMA = "lily_l4_b86r12_provisioning_report_v14"
SEAL = {"status": "sealed_not_accessed", "accessed": False}
HEX40 = re.compile(r"^[0-9a-f]{40}$")
HEX64 = re.compile(r"^[0-9a-f]{64}$")
MARKER_BYTES = b'{"schema_version":"lily_l4_b86r12_attempt_v14","state":"consumed"}'
...
def activation_content(gate, gate_raw, *, accepted_gate_head_sha, hermetic_ci_run_id, owner_authorization_reference):
    """Future activation code must take the schema version from the locked gate."""
    return {
        "schema_version": gate["activation_schema_version"],
...
def outputs_ok(manifest, payload):
    ...
    for symbol in U8:
        ...
        if not isinstance(row, dict) or set(row) != {"start", "end", "row_count"} or not isinstance(dates, list) or not dates or any(not d8(item) for item in dates) or dates != sorted(set(dates)) or not isinstance(row["row_count"], int) or row["row_count"] <= 0 or row != {"start": dates[0], "end": dates[-1], "row_count": len(dates)}:
            return False
        all_dates += dates
