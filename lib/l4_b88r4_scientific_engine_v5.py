"""Pure B8.8R4/v4 L-4 derivation from a normalized U8 container.
...
def drift_weights(weights: Sequence[float], asset_returns: Sequence[float], cash_return: float) -> list[float] | None:
    """One-close state transition, including the uninvested cash sleeve."""
    if len(weights) != len(asset_returns) or not all(_finite(value) for value in tuple(weights) + tuple(asset_returns) + (cash_return,)):
        return None
    cash_weight = max(0.0, 1.0 - sum(abs(value) for value in weights))
    portfolio_return = sum(weight * asset_return for weight, asset_return in zip(weights, asset_returns, strict=True)) + cash_weight * cash_return
    denominator = 1.0 + portfolio_return
    return None if denominator <= 0.0 else [weight * (1.0 + asset_return) / denominator for weight, asset_return in zip(weights, asset_returns, strict=True)]
...
def _run_universe(sessions: list[str], returns: dict[str, list[float]], cash: list[float], symbols: tuple[str, ...]) -> dict[int, dict[str, Any]] | None:
    state = [0.0] * len(symbols); output: dict[int, dict[str, Any]] = {}; daily: list[dict[str, float]] = []; daily_costs: list[dict[str, float]] = []; scheduled: dict[int, tuple[list[float], dict[str, bool], list[float], list[list[float]], float]] = {}
    ...
    for index in range(len(sessions)):
        ...
        drift = drift_weights(state, asset_returns, cash[index])
        if drift is None:
            return None
    ...
    return {"weekly": output, "daily": daily, "daily_costs": daily_costs}
...
def derive(container: dict[str, Any], *, config: dict[str, Any]) -> dict[str, Any] | None:
    ...
    for common_index, decision in enumerate(common):
        ...
        def booked(run, start): return {name: sum(run["daily_costs"][day][name] for day in range(start, end)) for name in ("commission","spread_slippage","sell_surcharge","expense_ratio","short_borrow","cash_yield","total")}
        observations.append({"date": left["decision_date"], "timing": {"decision_date": left["decision_date"], "execution_date": left["execution_date"], "realized_dates": left["realized_dates"]}, "ex_ante_hhi_delta": h4-h8, "realized_hhi_delta": r4-r8, "top_dependency_delta": d4-d8, "n_eff_delta": n8[0]-n4[0], "costs_u4": booked(run4,left["execution_index"]), "costs_u8": booked(run8,right["execution_index"]), "changes_u4": left["changes"], "changes_u8": right["changes"], "gross_u4": left["gross"], "gross_u8": right["gross"], "flags_u4": left["flags"], "flags_u8": right["flags"], "q_u8": right["q"], "net_pnl_contribution_u4": pnl4, "net_pnl_contribution_u8": pnl8, "state":{"u4":{"weights":left["weights"],"covariance":left["covariance"],"hhi":h4},"u8":{"weights":right["weights"],"covariance":right["covariance"],"hhi":h8}}, "regime": {"global_state": global_state, "volatility_tercile":_prior_volatility_tercile(returns, decision), "equity_synchronization": equity}})
    statistics = _statistics(observations); side_effects = _side_effects(observations); robustness = _robustness(observations, statistics)
    constraints_pass = side_effects.get("pass", False) and all(value.get("pass", False) for value in robustness.values())
    return {"weekly_observations": observations, "statistics": statistics, "robustness": robustness, "side_effects": side_effects, "regimes": _regimes(observations), "outcome": classify_outcome(statistics, constraints_pass=constraints_pass)}
...
def _side_effects(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    result={"evaluable": cost4 != 0 and gross4 != 0 and gross8 != 0 and turnover[0] != 0, "turnover_relative_increase": (turnover[1]/gross8)/(turnover[0]/gross4)-1 if turnover[0] else None, "cost_relative_increase": (cost8/gross8)/(cost4/gross4)-1 if cost4 else None, "flag_frequency_delta_percentage_points": {flag: 100*(sum(row["flags_u8"][flag] for row in rows)-sum(row["flags_u4"][flag] for row in rows))/len(rows) for flag in ("cap", "cash", "scale_down")}}
    result["pass"] = bool(result["evaluable"] and result["turnover_relative_increase"] <= .20 and result["cost_relative_increase"] <= .20 and all(value <= 10.0 for value in result["flag_frequency_delta_percentage_points"].values()))
    return result


def _robustness(rows: list[dict[str, Any]], statistics: dict[str, Any]) -> dict[str, Any]:
    ...
    required=statistics.get("ex_ante_hhi_delta",{}).get("mintrl",{}).get("falsify")
    if not isinstance(required, int):
        return {"best_market":{"pass":False},"best_trend_episode":{"pass":False}}
