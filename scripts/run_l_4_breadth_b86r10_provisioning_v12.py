"""B8.6R10 runtime; only loaded after v12 bootstrap verification."""
from __future__ import annotations
import sys
from pathlib import Path
def run_one_shot(*,root:Path,commit:str,dependency_identities:dict,activation:dict)->dict:
    if str(root) not in sys.path:sys.path.insert(0,str(root))
    from lib.l4_b86r10_contract_v12 import ACTIVATION,DATASET,EXPECTED_DATASET_SHA256,MANIFEST,MARKER,MAX_BYTES,PAYLOAD,REPORT,REPORT_SCHEMA,SEAL,U8,artifact,atomic_write_all,canonical,claim_once,sha256
    ...
    if blocker:report.update({"outcome":"provisioning_blocked","blocker":blocker})
        ...
        try:scanned=scan_dataset(raw,expected_sha256=EXPECTED_DATASET_SHA256)
            ...
            manifest={"schema_version":"lily_l4_b86r10_falsification_manifest_v12","dataset_reference":DATASET,"dataset_sha256":scanned["dataset_sha256"],"dataset_byte_count":scanned["dataset_byte_count"],"u8_members_in_order":list(U8),"coverage_by_symbol":scanned["coverage_by_symbol"],"session_count":scanned["session_count"],"max_session_date":scanned["max_session_date"],"validation_seal":SEAL};payload={"schema_version":"lily_l4_b86r10_u8_session_dates_v12","dataset_sha256":scanned["dataset_sha256"],"u8_members_in_order":list(U8),"session_dates_by_symbol":scanned["session_dates_by_symbol"]};mr,pr=canonical(manifest),canonical(payload);atomic_write_all(((root/MANIFEST,mr),(root/PAYLOAD,pr)));report.update({"outcome":"structural_provisioned","manifest":manifest,"payload":payload,"output_artifacts":{"manifest":{"path":MANIFEST,"raw_sha256":sha256(mr),"byte_count":len(mr)},"payload":{"path":PAYLOAD,"raw_sha256":sha256(pr),"byte_count":len(pr)}},"structural_summary_sha256":sha256(canonical({"manifest":manifest,"payload":payload}))})
    atomic_write_all(((root/REPORT,canonical(report)),));return report
if __name__=="__main__":raise SystemExit(2)
