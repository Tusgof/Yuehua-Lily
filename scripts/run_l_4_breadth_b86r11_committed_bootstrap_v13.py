"""Stdlib-only, commit-sourced production bootstrap for B8.6R11/v13."""
from __future__ import annotations
import hashlib, json, runpy, subprocess, sys
from pathlib import Path

GATE = "experiments/l_4_breadth_b86r11_provisioning_gate_v13.json"; ACTIVATION = "experiments/activation_records/l_4_breadth_b86r11_provisioning_activation_v13.json"; RUNTIME = "scripts/run_l_4_breadth_b86r11_provisioning_v13.py"; GATE_ID = "l_4_breadth_b86r11_provisioning_gate_v13"; SEAL = {"status":"sealed_not_accessed","accessed":False}
DEPENDENCIES = (GATE, "scripts/run_l_4_breadth_b86r11_committed_bootstrap_v13.py", RUNTIME, "lib/l4_b86r11_contract_v13.py", "lib/l4_b86r2_provisioning_scanner_v3.py", "lib/draft202012_subset.py", "scripts/validate_l_4_breadth_b86r11_provisioning_gate_v13.py", "scripts/validate_l_4_breadth_b86r11_provisioning_report_v13.py", "schemas/l_4_breadth_b86r11_provisioning_activation_v13.schema.json", "schemas/l_4_breadth_b86r11_provisioning_report_v13.schema.json", "schemas/l_4_breadth_b86r11_falsification_manifest_v13.schema.json", "schemas/l_4_breadth_b86r11_u8_session_dates_v13.schema.json")
...
def activation_ok(root, commit, gate_raw):
    ...
    keys = {"schema_version", "gate_id", "gate_sha256", "accepted_gate_head_sha", "hermetic_ci_head_sha", "hermetic_ci_run_id", "inspector_decision", "owner_authorization_reference", "scope", "validation_seal"}; accepted = value.get("accepted_gate_head_sha")
    if raw != canonical(value) or set(value) != keys or value.get("schema_version") != "lily_l4_b86r11_provisioning_activation_v13" or value.get("gate_id") != GATE_ID or value.get("gate_sha256") != sha256(gate_raw) or not h64(value.get("gate_sha256")) or not h40(accepted) or value.get("hermetic_ci_head_sha") != accepted or not isinstance(value.get("hermetic_ci_run_id"), int) or isinstance(value.get("hermetic_ci_run_id"), bool) or value["hermetic_ci_run_id"] <= 0 or value.get("inspector_decision") != "ACCEPTED" or value.get("owner_authorization_reference") != "B8.6R11 one-shot owner authorization" or value.get("scope") != "one_repo_relative_falsification_container_provisioning_only" or value.get("validation_seal") != SEAL: return None
    ancestor = subprocess.run(["git", "merge-base", "--is-ancestor", accepted, commit], cwd=root, capture_output=True, check=False)
    return None if ancestor.returncode != 0 or blob(root, accepted, GATE) != gate_raw else {"path": ACTIVATION, "raw_sha256": sha256(raw), "content": value, "activation_checkpoint_head": commit, "accepted_gate_blob_sha256": sha256(gate_raw)}
def run(root, commit):
