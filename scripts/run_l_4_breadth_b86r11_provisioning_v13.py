"""B8.6R11 runtime; loaded only after v13 provenance verification."""
from __future__ import annotations
import sys
from pathlib import Path
def run_one_shot(*, root: Path, commit: str, dependency_identities: dict, activation: dict) -> dict:
    if str(root) not in sys.path: sys.path.insert(0, str(root))
    from lib.l4_b86r11_contract_v13 import DATASET, EXPECTED_DATASET_SHA256, MANIFEST, MARKER, MAX_BYTES, PAYLOAD, REPORT, REPORT_SCHEMA, SEAL, U8, artifact, atomic_write_all, canonical, claim_once, sha256
