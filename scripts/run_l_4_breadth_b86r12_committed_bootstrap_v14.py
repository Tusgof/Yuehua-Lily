"""Stdlib-only, commit-sourced B8.6R12/v14 bootstrap.

The gate owns the activation schema version.  This module must not import project
code until ``preflight`` returns ``ready=True``.
"""
...
RUNTIME = "scripts/run_l_4_breadth_b86r12_provisioning_v14.py"
GATE_ID = "l_4_breadth_b86r12_provisioning_gate_v14"
SEAL = {"status": "sealed_not_accessed", "accessed": False}
DEPENDENCIES = (
    GATE,
    "scripts/run_l_4_breadth_b86r12_committed_bootstrap_v14.py",
    RUNTIME,
    "lib/l4_b86r12_contract_v14.py",
    "lib/l4_b86r2_provisioning_scanner_v3.py",
    "lib/draft202012_subset.py",
    "scripts/validate_l_4_breadth_b86r12_provisioning_gate_v14.py",
    "scripts/validate_l_4_breadth_b86r12_provisioning_report_v14.py",
    "scripts/validate_l_4_breadth_b86r12_provisioning_activation_v14.py",
    "schemas/l_4_breadth_b86r12_provisioning_activation_v14.schema.json",
    "schemas/l_4_breadth_b86r12_provisioning_report_v14.schema.json",
    "schemas/l_4_breadth_b86r12_falsification_manifest_v14.schema.json",
    "schemas/l_4_breadth_b86r12_u8_session_dates_v14.schema.json",
)
...
def gate_ok(root, commit, current):
    ...
    schema = gate.get("activation_schema_version")
    if gate.get("gate_id") != GATE_ID or gate.get("execution_dependencies") != list(DEPENDENCIES) or gate.get("execution_binding") != expected or not isinstance(schema, str) or not schema:
        return None
    return raw, gate
...
def preflight(root, producing_commit):
    """Run the sole identities -> gate -> activation path, before project imports."""
    root = Path(root).resolve()
    current = identities(root, producing_commit)
    if current is None:
        return {"ready": False, "outcome": "refused_execution_provenance", "dataset_read_count": 0}
    resolved_gate = gate_ok(root, producing_commit, current)
    if resolved_gate is None:
        return {"ready": False, "outcome": "refused_execution_provenance", "dataset_read_count": 0}
    raw, gate = resolved_gate
    activation = activation_ok(root, producing_commit, raw, gate)
    if activation is None:
        return {"ready": False, "outcome": "refused_activation", "dataset_read_count": 0}
...
def main(argv):
    if len(argv) != 5 or argv[:2] != ["--committed-bootstrap", "--repo-root"] or argv[3] != "--producing-commit" or Path(sys.argv[0]).name != "-":
        return 2
