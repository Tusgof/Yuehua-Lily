"""Real future v5 runtime; it is intentionally unreachable in E0 Phase A."""
...
from lib.l4_b88r4_lifecycle_v5 import GATE, MARKER, canonical, sha
...
def _claim_marker(root: Path, preflight: dict) -> Path | None:
    ...
    with os.fdopen(descriptor, "wb") as handle:
        handle.write(canonical({"schema_version":"lily_l4_b88r4_marker_v5","producing_commit":preflight["producing_commit"],"activation_sha256":sha(canonical(preflight["activation"]))}))
...
def run_one_shot(preflight: dict, *, root: Path) -> dict:
    ...
    outputs = tuple(root / name for name in ("reports/experiments/l_4_breadth_b88r4_scientific_report_v5.json", "reports/experiments/l_4_breadth_b88r4_execution_ledger_v5.json", "reports/experiments/l_4_breadth_b88r4_execution_attempt_v5.json"))
    ...
    output.write_bytes(canonical(report)); ledger.write_bytes(canonical({"schema_version":"lily_l4_b88r4_execution_ledger_v5","report_sha256":sha(canonical(report)),"marker_path":MARKER,"marker_sha256":hashlib.sha256(marker.read_bytes()).hexdigest(),"container_sha256":activation["container_sha256"]})); attempt.write_bytes(canonical({"schema_version":"lily_l4_b88r4_execution_attempt_v5","status":"completed_once","marker_path":MARKER,"report_sha256":sha(canonical(report))}))
