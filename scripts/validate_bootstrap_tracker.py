...
def _validate_done_artifact(
    ...
    *,
    ...
    if must == "validate_core_1e_b2_gate":
        if not target.is_file():
            return [f"{order_id}:missing_artifact:{artifact_path}"], False, False
        completed = subprocess.run(
            [sys.executable, "scripts/validate_core_1e_b2_development_execution_contract_v1.py"],
            cwd=project_root,
            text=True,
            capture_output=True,
            check=False,
        ...
        return ([] if ok else [f"{order_id}:core_1e_b2_contract_validator_failed"], ok, False)
    if must == "validate_core_1e_b2_report":
        if not target.is_file():
            return [f"{order_id}:missing_artifact:{artifact_path}"], False, False
        completed = subprocess.run(
            [sys.executable, "scripts/validate_core_1e_b2_empirical_report_v1.py"],
            cwd=project_root,
            text=True,
            capture_output=True,
            check=False,
        ...
        ok = completed.returncode == 0
        return ([] if ok else [f"{order_id}:core_1e_b2_report_validator_failed"], ok, False)
    if must == "deny_core_1e_b2_bootstrap":
        if not target.is_file():
            return [f"{order_id}:missing_artifact:{artifact_path}"], False, False
        completed = subprocess.run(
        ...
        try:
            ...
            ok = False
        return ([] if ok else [f"{order_id}:core_1e_b2_bootstrap_not_deny_only"], ok, False)
    if must == "contain_core_1e_b2_manifest":
        try:
            rows = [json.loads(line) for line in target.read_text(encoding="utf-8").splitlines() if line.strip()]
            matches = [row for row in rows if row.get("gate_id") == "core_1e_b2_development_execution_contract_v1"]
            gate = project_root / "experiments/core_1e_b2_development_execution_contract_v1.json"
            validator = project_root / "scripts/validate_core_1e_b2_development_execution_contract_v1.py"
            ok = (
                len(matches) == 1
                and matches[0].get("artifact_path") == "experiments/core_1e_b2_development_execution_contract_v1.json"
                and matches[0].get("artifact_sha256") == hashlib.sha256(gate.read_bytes()).hexdigest()
                and matches[0].get("validator_path") == "scripts/validate_core_1e_b2_development_execution_contract_v1.py"
                and matches[0].get("validator_sha256") == hashlib.sha256(validator.read_bytes()).hexdigest()
            )
        except (OSError, json.JSONDecodeError, TypeError, KeyError):
            ok = False
        return ([] if ok else [f"{order_id}:core_1e_b2_manifest_identity_failed"], ok, False)
    if must == "register_core_1e_b2_scripts":
        required = {
            "scripts/validate_core_1e_b2_development_execution_contract_v1.py",
            "scripts/validate_core_1e_b2_empirical_report_v1.py",
            "scripts/run_core_1e_b2_committed_bootstrap_v1.py",
        }
        try:
            config = json.loads(target.read_text(encoding="utf-8"))
            registered = set(config.get("scripts", [])) | set(config.get("stdlib_only_scripts", []))
            ok = required <= registered
        except (OSError, json.JSONDecodeError, TypeError):
            ok = False
        return ([] if ok else [f"{order_id}:core_1e_b2_script_registration_failed"], ok, False)
    if must == "pass_core_1e_b2_hermetic":
        if not target.is_file():
            return [f"{order_id}:missing_artifact:{artifact_path}"], False, False
        completed = subprocess.run(
            [sys.executable, "-m", "unittest", "tests.test_core_1e_b2_synthetic_machinery"],
            cwd=project_root,
            text=True,
            capture_output=True,
            check=False,
    ...
    if must == "deny_l4_b88r2_without_activation":
        if not target.is_file(): return [f"{order_id}:missing_artifact:{artifact_path}"], False, False
        if not verify_runtime: return [], False, True
        completed = subprocess.run([sys.executable, str(target)], cwd=project_root, text=True, capture_output=True, check=False)
    ...
    if must == "validate_l4_b88r5ar_activation":
        if not target.is_file():
            return [f"{order_id}:missing_artifact:{artifact_path}"], False, False
        if not verify_runtime:
            return [], False, True
        ...
        ok = (
            ...
            and result.get("preflight_outcome") == "preflight_ready"
            and result.get("real_accessed") is False
        )
        return (
            [] if ok else [f"{order_id}:l4_b88r5ar_activation_validation_failed"],
            ok,
            False,
        )
    if must == "register_l4_b88r5ar_scripts":
        required = ("scripts/validate_l_4_breadth_b88r5ar_activation_v6.py",)
        try:
            scripts = json.loads(target.read_text(encoding="utf-8")).get("scripts", [])
            ok = all(scripts.count(item) == 1 for item in required)
        except Exception:
            ok = False
        return (
            [] if ok else [f"{order_id}:l4_b88r5ar_script_registration_mismatch"],
            ok,
            False,
        )
    ...
    if must == "match_l4_b88r5ar_mirror":
        try:
            ...
            if artifact_path == "experiments/hypothesis_registry.json":
                l4 = next(item for item in json.loads(text).get("hypotheses", []) if item.get("id") == "L-4")
                ok = ok and any(
                    item.get("decision") == "B8_8R5AR_clean_replacement_activation_checkpoint_E0"
                    for item in l4.get("decision_log", [])
                    if isinstance(item, dict)
                )
                ok = ok and any(
                    item.get("path") == "experiments/activation_records/l_4_breadth_b88r5_scientific_execution_activation_v6.json"
                    and item.get("evidence_tier") == "E0"
                    for item in l4.get("evidence", [])
                    if isinstance(item, dict)
                )
        except Exception:
            ok = False
