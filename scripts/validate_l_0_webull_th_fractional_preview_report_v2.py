...
AUTH_CREATE_PATH = "/openapi/auth/token/create"
AUTH_CHECK_PATH = "/openapi/auth/token/check"
PREVIEW_PATH = "/openapi/trade/order/preview"
HASH_PATTERN = re.compile(r"^[0-9a-f]{64}$")
COMMIT_PATTERN = re.compile(r"^[0-9a-f]{40}$")
CLAIM_LIMITS = [
    "The report measures tested VTI acceptance on the shared Thailand UAT environment only; it does not prove an order was placed or filled.",
    "The result does not establish an exact broker minimum and may not be generalized to other symbols or production.",
    "The report provides no execution-quality, strategy-edge, E2, deployment, paper-trading, or real-money evidence.",
]
...
def validate_report(path: Path) -> dict[str, Any]:
    ...
    if mode == "uat_preview" and report.get("activation_gate_id") != "l_0_webull_th_fractional_preview_activation_v2":
    ...
    for index, row in enumerate(rows):
        ...
        if not isinstance(row, dict):
            ...
            continue
        quantity = QUANTITY_GRID[index]
        if row.get("quantity") != quantity:
            blockers.append(f"{label}:quantity_order_mismatch")
        if row.get("outcome") == "accepted":
            if row.get("response_class") != "success" or row.get("documented_error_code") is not None:
                blockers.append(f"{label}:accepted_shape_mismatch")
            accepted.append(Decimal(quantity))
        elif row.get("outcome") == "rejected":
            if row.get("response_class") != "business_error" or not row.get("documented_error_code"):
                blockers.append(f"{label}:rejected_shape_mismatch")
            rejected.append(Decimal(quantity))
        else:
    ...
    _validate_attestation(report.get("request_attestation"), mode, len(rows), bool(declared_blockers), blockers)
...
def _validate_attestation(
    ...
    if not isinstance(paths, list) or any(path not in {AUTH_CREATE_PATH, AUTH_CHECK_PATH} for path in paths):
        blockers.append("authentication_path_inventory_invalid")
        paths = []
    create_count = paths.count(AUTH_CREATE_PATH)
    check_count = paths.count(AUTH_CHECK_PATH)
    preview_count = attestation.get("preview_request_count")
    if mode == "uat_preview":
        if not isinstance(preview_count, int) or preview_count < row_count or preview_count > row_count + 1:
            blockers.append("request_attestation_mismatch:preview_request_count")
        elif preview_count != row_count and not has_blockers:
            blockers.append("unclassified_preview_request_requires_blocker")
    elif paths or preview_count != 0:
        blockers.append("synthetic_fixture_must_have_no_broker_requests")
    expected_preview_count = preview_count if isinstance(preview_count, int) else 0
    expected = {
        "authentication_request_count": len(paths) if mode == "uat_preview" else 0,
        "auth_create_request_count": create_count if mode == "uat_preview" else 0,
        "auth_check_request_count": check_count if mode == "uat_preview" else 0,
        "preview_path": PREVIEW_PATH,
        "synthetic_preview_row_count": row_count if mode == "synthetic_fixture" else 0,
        "total_broker_request_count": (len(paths) + expected_preview_count) if mode == "uat_preview" else 0,
    ...
    if create_count > 1 or check_count > 7 or len(paths) > 8 or expected_preview_count > 8:
        blockers.append("request_cap_exceeded")
    if len(paths) + expected_preview_count > 16:
        blockers.append("total_request_cap_exceeded")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate the B4.12 Webull Thailand UAT preview report.")
