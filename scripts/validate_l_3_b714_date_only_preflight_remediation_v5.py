"""Fail-closed validator for append-only B7.14R3/v5 E0 gate."""
...
GATE = ROOT / "experiments/l_3_b714_date_only_preflight_remediation_v5.json"
...
def validate() -> dict[str, object]:
    ...
    identity = {"schema_version": "lily_l3_b714_date_only_preflight_remediation_v5", "order_id": "B7.14R3", "gate_id": "l_3_b714_date_only_preflight_remediation_v5", "supersedes_gate_id": "l_3_b714_date_only_preflight_remediation_v4", "hypothesis_id": "L-3", "status": "locked_E0_rejected_v4_remediation", "evidence_ceiling": "E0", "edge_claim": "none", "validation_seal": {"status": "sealed_not_accessed", "accessed": False}}
    ...
    if not isinstance(source, dict) or set(source) != expected_source_keys: blockers.append("source_shape")
        ...
        if source["v4"].get("manifest_gate_id") != "l_3_b714_date_only_preflight_remediation_v4" or not any(row.get("gate_id") == "l_3_b714_date_only_preflight_remediation_v4" and row.get("artifact_sha256") == SOURCES["v4"][1] for row in rows): blockers.append("v4_manifest_identity")
    ...
    if not isinstance(row, dict) or row.get("artifact_sha256") != file_sha256(GATE) or row.get("validator_sha256") != file_sha256(Path(__file__)) or row.get("human_approval") is None or row.get("reviewed_by") is None: blockers.append("v5_manifest_identity")
