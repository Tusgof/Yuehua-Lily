"""Closed-world E0 v6 report validator for committed synthetic fixtures only."""
...
from lib.l3_b714_date_only_scanner_v6 import ASSETS, scan_synthetic_date_only
from lib.provenance import file_sha256

FIXTURE_ROOT = ROOT / "tests/fixtures/l3_b714_v6"
...
ARTIFACTS = {"scanner": "lib/l3_b714_date_only_scanner_v6.py", "runner": "scripts/run_l_3_b714_date_only_preflight_v6.py", "report_schema": "schemas/l_3_b714_date_only_preflight_report_v6.schema.json", "attestation_schema": "schemas/l_3_b714_date_only_schedule_attestation_v6.schema.json", "report_validator": "scripts/validate_l_3_b714_date_only_preflight_report_v6.py"}
...
def _git_tree_contains(commit: str) -> bool:
    ...
    if probe.returncode:
        return False
    for path in ("experiments/l_3_b714_date_only_preflight_remediation_v6.json", *ARTIFACTS.values(), "experiments/locked_gates.jsonl"):
        ...
        if shown.returncode or shown.stdout != (ROOT / path).read_bytes():
            return False
    return b'l_3_b714_date_only_preflight_remediation_v6' in subprocess.run(["git", "show", f"{commit}:experiments/locked_gates.jsonl"], cwd=ROOT, capture_output=True, check=False).stdout
...
def validate(report_path: Path = REPORT, attestation_path: Path | None = ATTESTATION) -> dict[str, object]:
    ...
    if not _closed(metadata, {"schema_version", "common_sessions"}) or metadata.get("schema_version") != "lily_l3_b714_synthetic_date_metadata_v6" or not isinstance(metadata.get("common_sessions"), list):
    ...
    metadata_identity = {"path": "tests/fixtures/l3_b714_v6/metadata.json", "sha256": file_sha256(METADATA)}
    ...
    if {key: report.get(key) for key in ("schema_version", "order_id", "hypothesis_id", "evidence_tier", "edge_claim")} != {"schema_version": "lily_l3_b714_date_only_preflight_report_v6", "order_id": "B7.14R4", "hypothesis_id": "L-3", "evidence_tier": "E0", "edge_claim": "none"}: blockers.append("report_identity")
    ...
    if report.get("mode") == "synthetic_preflight_pass":
        expected_attestation = {"schema_version": "lily_l3_b714_date_only_schedule_attestation_v6", "order_id": "B7.14R4", "hypothesis_id": "L-3", "evidence_tier": "E0", "edge_claim": "none", "mode": "synthetic_preflight_pass", "metadata": metadata_identity, "schedule": schedule, "validation_seal": SEAL, "access_counters": ZERO}
        if attestation != expected_attestation: blockers.append("attestation_content")
        if attestation_path is None or report.get("attestation") != {"path": "tests/fixtures/l3_b714_v6/attestation.json", "sha256": file_sha256(attestation_path)}: blockers.append("attestation_identity")
