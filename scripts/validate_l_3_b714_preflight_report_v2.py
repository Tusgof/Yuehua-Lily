...
def validate(payload: Any) -> dict[str, Any]:
    ...
    gate = json.loads((ROOT / "experiments/l_3_b714_activation_contract_v2.json").read_text())
    approved = gate["approved_synthetic_metadata"]
    expected_gate = file_sha256(ROOT / "experiments/l_3_b714_activation_contract_v2.json")
    expected_metadata = approved["canonical_metadata_sha256"]
    if canonical_sha256(payload.get("synthetic_date_metadata")) != expected_metadata or file_sha256(ROOT / approved["path"]) != approved["sha256"] or not isinstance(provenance, dict) or set(provenance) != {"active_gate_id", "active_gate_sha256", "fixture_metadata_sha256"} or provenance.get("active_gate_id") != "l_3_b714_activation_contract_v2" or provenance.get("active_gate_sha256") != expected_gate or provenance.get("fixture_metadata_sha256") != expected_metadata:
