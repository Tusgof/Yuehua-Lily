"""Closed-world B8.6R13/v15 report and output validator."""
...
from lib.l4_b86r13_contract_v15 import BLOCKERS, COMMON_REPORT_KEYS, MANIFEST, PAYLOAD, REPORT_SCHEMA, SEAL, artifact, canonical, h40, outputs_ok, row_ok, sha256
from scripts.run_l_4_breadth_b86r13_committed_bootstrap_v15 import DEPENDENCIES, preflight
...
def artifacts_ok(report, root):
    commit, identities = report.get("producing_git_commit"), report.get("contract_artifacts")
    return h40(commit) and isinstance(identities, dict) and set(identities) == set(DEPENDENCIES) and all((raw := blob(root, commit, path)) is not None and identities[path] == {"path": path, "sha256": sha256(raw)} for path in DEPENDENCIES)


def activation_ok(report, root):
    commit = report.get("producing_git_commit")
    if not h40(commit): return False
    checked = preflight(root, commit)
    return checked.get("ready") is True and report.get("activation_provenance") == checked["activation"]


def outputs_identity_ok(report, root):
    ...
    for name, path, value in (("manifest", MANIFEST, manifest), ("payload", PAYLOAD, payload)):
        ...
        if identities[name] != {"path": path, "raw_sha256": sha256(raw), "byte_count": len(raw)} or disk != raw: return False
    return True
...
def validate(report, *, root=ROOT):
    ...
    if not expected or not isinstance(report, dict) or set(report) != expected: blockers.append("closed_world")
    if mode == "synthetic_fixture":
        ...
        if report.get("real_provisioning_consumed") is not True or not artifacts_ok(report, root) or not activation_ok(report, root): blockers.append("provenance")
...
if __name__ == "__main__":
    try: value = json.loads(Path(sys.argv[1]).read_text("ascii"))
    except (IndexError, OSError, ValueError): raise SystemExit(2)
    result = validate(value); print(json.dumps(result)); raise SystemExit(result["status"] != "pass")
