"""Validate v5's closed E0 gate without touching a market container."""
...
GATE = ROOT / "experiments/l_4_breadth_b88r4_phase_a_execution_contract_v5.json"
REQUIRED = {"schema_version", "order_id", "gate_id", "supersedes_gate_id", "hypothesis_id", "status", "evidence_ceiling", "edge_claim", "v4_rejection", "owner_literal", "sources", "science", "activation", "execution_dependencies", "execution_binding", "validation_seal", "authorizations", "access_counts", "hard_stops"}
...
def validate() -> dict:
    ...
    if {key:value.get(key) for key in ("schema_version","order_id","gate_id","supersedes_gate_id","hypothesis_id","evidence_ceiling","edge_claim","owner_literal")} != {"schema_version":"lily_l4_b88r4_phase_a_execution_contract_v5","order_id":"B8.8R4","gate_id":"l_4_breadth_b88r4_phase_a_execution_contract_v5","supersedes_gate_id":"l_4_breadth_b88r3_phase_a_execution_contract_v4","hypothesis_id":"L-4","evidence_ceiling":"E0","edge_claim":"none","owner_literal":"continue the work till we complete L4"}: blockers.append("identity")
