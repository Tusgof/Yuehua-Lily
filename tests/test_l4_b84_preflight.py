...
from scripts.run_l_4_breadth_b84_preflight_v1 import run
from scripts.validate_l_4_breadth_b84_preflight_report_v1 import materialize_fixture, validate

ROOT = Path(__file__).resolve().parents[1]
FIXTURE = ROOT / "tests/fixtures/l4_b84/synthetic_preflight_report.json"
...
class B84Tests(unittest.TestCase):
    ...
    def test_runner_rejects_nonfixture_and_tampered_fixture(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            forged = Path(tmp) / "synthetic_preflight_report.json"
            ...
            with patch("scripts.run_l_4_breadth_b84_preflight_v1.FIXTURE", forged):
                self.assertEqual("blocked", run(forged)["status"])
