...
from scripts.run_l_4_breadth_b84r2_preflight_v3 import run
from scripts.validate_l_4_breadth_b84r2_preflight_report_v3 import materialize_fixture, validate
from scripts.validate_l_4_breadth_b84r2_activation_contract_v3 import canonical_sha, validate as validate_gate

ROOT = Path(__file__).resolve().parents[1]
FIXTURE = ROOT / "tests/fixtures/l4_b84/synthetic_preflight_report_v3.json"
...
class B84Tests(unittest.TestCase):
    ...
    def test_runner_rejects_nonfixture_and_tampered_fixture(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            forged = Path(tmp) / "synthetic_preflight_report_v3.json"
            ...
            with patch("scripts.run_l_4_breadth_b84r2_preflight_v3.FIXTURE", forged):
                self.assertEqual("blocked", run(forged)["status"])

    def test_gate_source_and_eol_tamper_fail_closed(self) -> None:
        self.assertEqual("pass", validate_gate()["status"])
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "x.txt"
            path.write_bytes(b"a\r\nb\r\n")
            crlf = canonical_sha(path)
            path.write_bytes(b"a\nb\n")
            self.assertEqual(crlf, canonical_sha(path))
            path.write_bytes(b"a\nc\n")
            self.assertNotEqual(crlf, canonical_sha(path))
            gate = ROOT / "experiments/l_4_breadth_b84r2_activation_contract_v3.json"
            for mutate in (
                lambda p: p["source_binding"]["v4_active_science"].__setitem__("artifact_sha256", "0" * 64),
                lambda p: p["source_binding"]["b84_v1_failed_history"].__setitem__("ci_run", "0"),
                lambda p: p["source_binding"].__setitem__("extra", {}),
